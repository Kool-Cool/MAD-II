<template>
    <div class="login container mt-5">
      ...
      <button @click="$router.push('/influencer/register')" class="btn btn-secondary btn-block mt-3">New influencer Registration</button>
    </div>
</template>
  ...
  export default {
    ...
    methods: {
      async login() {
        try {
          const response = await axios.post('http://localhost:5000/influencer/login', {
