...
<template>
    ...
    <div>
      ...
      <nav class="navbar navbar-expand-lg navbar-light bg-light">
        <button class="btn btn-primary" @click="navigateToPublicCampaigns">See All Public Campaigns</button>
      </nav>
  </div>   
      ...
      <table v-if="data.length" class="table table-striped table-bordered">
        ...
        <tbody>
          <tr v-for="item in data" :key="item.ad_request_id">
            ...
            <td>
              <button class="btn btn-success" @click="acceptAdRequest(item.ad_request_id)">Accept</button>
              <button class="btn btn-danger" @click="rejectAdRequest(item.ad_request_id)">Reject</button>
              <button class="btn btn-warning" @click="negotiateAdRequest(item.ad_request_id)">Negotiate</button>
            </td>
          </tr>
        </tbody>
      </table>
