...
<template>
  <Logout DashboardTitle="Admin Dashboard"></Logout>
    <div class="pending-sponsors container mt-5">
      ...
      <div v-if="sponsors.length" class="list-group">
        <div v-for="sponsor in sponsors" :key="sponsor.sponsor_id" class="list-group-item">
          <h5 class="mb-1">{{ sponsor.company_name }}</h5>
          <p class="mb-1">Industry: {{ sponsor.industry }}</p>
          <button @click="approveSponsor(sponsor.sponsor_id)" class="btn btn-primary">Approve</button>
        </div>
      </div>
  ...
  export default {
    ...
    async created() {
      try {
        const response = await axios.get('http://localhost:5000/admin/pending_sponsors', {
    ...
    methods: {
      async approveSponsor(sponsorId) {
        try {
          await axios.post(`http://localhost:5000/admin/approve_sponsor/${sponsorId}`, {}, {
            headers: { 'Authorization': `Bearer ${this.$store.state.token}` }
          });
